...
本地 `prts_animation_validator.html` 通过现有 Windows Headless Chrome 150 和 WSL `playwright-cli` CDP 路径读取当前 PRTS 资源。`prts_animation_validation_report.json` 记录 17/17 用例通过、0 失败；16 个正向用例均为 0 失败请求，另有 1 个缺失动画负例：
...
| 阿米娅、德克萨斯、凯尔希 默认/基建与正面 | 基建包含 `Interact / Move / Relax / Sit / Sleep`；正面包含 `Attack / Skill / Die / Stun` 的不同子集；三组稳定交集只有 `Default` |
...
第一轮联网验证遇到的原始错误为 `net::ERR_CONNECTION_CLOSED`。网页客户端增加 3 次有界重试后，最终 16 个正向用例的 `failed_requests` 均为 0。以上结果证明网页侧 PRTS WebGL 资源与诊断逻辑；ONI C# mesh 的视觉等价性仍需游戏内验证。
...
- 存档内选中复制人后按 `Ctrl+F8` 打开单独外观选择器，`Ctrl+Shift+F8` 打开全局 Options；保存后目标 overlay 取消旧请求，在新骨骼完整解析后替换资源，失败时保留当前外观。
...
`0.3.3` Stable 在上述 Alpha 基线上新增用户可调缓存合同：`CacheCapacityMiB` 只接受 `128–2000` 的整数，默认和旧配置迁移值为 `512`；永久模式禁用输入并保留数值；保存更小容量后立即执行 LRU，同时保护当前使用、在途下载和持有租约的资源。`64 MiB` 单个 Spine 源文件限制与 `512 MiB` 备用包安全上限继续固定。配置界面和持久化已完成维护者实机确认，长期缓存压力项继续按 [第一阶段架构与验收规范](./phase1_architecture_and_acceptance.md) 的 `CAC-008` 至 `CAC-010` 单独记录。

游戏内可玩主路径已于 2026-07-15 验证：Alpha 候选 `0.3.2-alpha.1` 从 Steam 正常冷启动并加载，Options 五行布局、中英日名称与重定向别名搜索、皮肤/模型联动和逐复制人 `Ctrl+F8` 均可用；同一存档的四个复制人分别使用德克萨斯、阿米娅、凯尔希和能天使，保存重载后保持各自选择。稳定参考动画校准修复了视觉高度和脚底基线，移动、工作、攻击、睡眠、眩晕和死亡语义动作已通过实机转盘与日志观察。C# mesh 尚未实现 Spine 双色 shader；非黑双色模型的 Chrome 按需烘焙并加载 sheet 已列为后续复杂视觉增强，不计入本轮第一阶段代码完成状态。
...
- 已完成：全局默认和逐复制人独立选择的跨存档重载持久化；继续验证资源租约在返回主菜单后的释放。
...
| 7 个模型样本、16 个实时动画/特征用例和 1 个缺失动画负例 | 已验证 | `preview/prts_animation_validation_report.json`，Playwright 17/17 通过 |
