...
`0.3.3` 开发线在上述 Alpha 基线上新增用户可调缓存合同：`CacheCapacityMiB` 只接受 `128–2000` 的整数，默认和旧配置迁移值为 `512`；永久模式禁用输入并保留数值；保存更小容量后立即执行 LRU，同时保护当前使用、在途下载和持有租约的资源。`64 MiB` 单个 Spine 源文件限制与 `512 MiB` 备用包安全上限继续固定。代码与回归测试已在 `develop` 完成，实机验证仍待执行；当前 Stable 行为保持不变。验收状态以 [第一阶段架构与验收规范](./phase1_architecture_and_acceptance.md) 的 `CAC-008` 至 `CAC-010` 为准。
