...
6. 当前外观的 atlas、skel 和纹理全部校验后写入共享缓存，并沿用用户设置的按需 LRU 容量或永久保留策略。`0.3.3` 的按需容量范围为 `128–2000 MiB`，默认 `512 MiB`。
...
100 MB 是本机磁盘空间偏好。GitHub runner 对达到 100 MiB 的单干员包输出警告并继续；运行时对 ZIP 保留 `512 MiB` 的异常响应/压缩炸弹技术上限。当前单个 Spine 源文件仍使用既有 `64 MiB` 安全上限。用户设置的 `128–2000 MiB` LRU 容量只控制成功写入后的本地缓存维护，不改变这两项下载安全护栏。若真实干员包触发 `512 MiB` 上限，需要按该干员的外观拆包并升级 manifest schema。
...
- 每个干员 ZIP 通过 `512 MiB` 异常响应/压缩炸弹技术护栏；达到 100 MiB 的报告逐项人工确认，单纯超过本机偏好不会阻断云端工作流。用户可调缓存容量不能绕过该护栏。
...
- 未发布草稿：删除草稿和未使用 tag 即可，不影响当前公开的 `v0.3.2-alpha.2` 用户。
