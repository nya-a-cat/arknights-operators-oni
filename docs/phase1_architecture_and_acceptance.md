...
`0.3.3` 的容量输入只接受整数。空值、文本以及小于 `128` 或大于 `2000` 的值阻止保存，并显示 `128–2000 MiB` 范围提示。界面同时显示“当前占用 / 目标容量”。该合同从 `0.3.3` Stable 起生效。
...
- **`0.3.3` 已进入 Stable**：`128–2000 MiB` 整数容量、默认与迁移值 `512 MiB`、当前占用/目标容量、永久模式禁用并保留容量、下调容量立即维护及保护资源超标提示；配置界面和持久化已完成维护者实机确认，长期缓存压力项继续按本规范单独记录。
