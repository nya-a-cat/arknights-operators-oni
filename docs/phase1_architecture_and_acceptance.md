...
当前证据状态：**可玩主路径已通过游戏内验收，第一阶段的长期缓存与压力项仍待补测。** 已通过零警告 DLL 编译、31 项动画映射断言、21 项外观目录断言、10 项语言断言、8 项缓存索引断言、四路冷缓存与调用者独立取消解析、同长度缓存损坏恢复和 Playwright 12/12；ONI 模组页 Options、中英日名称与重定向别名搜索、皮肤/模型联动、`Ctrl+F8`、四个复制人实时切换、脚底对齐、移动/爬梯与配置持久化均有画面和 `Player.log` 证据。离线首次失败、永久策略、20 个复制人压力和引用释放仍保持待验证。旧版 130 个大图 sheet 只保留为原型历史。
...
`OperatorAnimationMapper.cs` 已实现动作分类、精确匹配、子串匹配、首动画降级和 `OperatorAnimationPlan`。`OperatorAnimationMapperTests.cs` 的 31 项断言已在 2026-07-15 重新运行，输出为：

```text
OperatorAnimationMapperTests: 31 passed
...
| --- | --- | --- | --- | --- |
| WEB-001 | 页面启动 | 本地 HTTP + 当前 PRTS | 12 个用例完成；正向用例无诊断错误和失败请求，负例产生规定错误码 | 已验证 |
...
| LIVE-001 | PRTS 联网冒烟 | 4 个角色、11 个正向动画/特征用例和 1 个负例 | 正向资源全部加载且无失败请求；负例输出稳定错误码 | 12/12 已验证 |
...
- 结果：12/12 通过，0 失败；其中 11 个正向资源/动画用例和 1 个缺失动画负例。
