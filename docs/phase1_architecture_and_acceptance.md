...
- 当前 Mod 根目录：`arknights_oni_mod_work/AmiyaDuplicantMod`
...
任何单个依赖、安装包或网络对象不得超过 100 MB。开始获取前必须完成以下只读检查：

1. 记录官方来源页面或官方仓库。
2. 读取并记录 license；license 缺失或含义不清时停止获取。
3. 通过 `Content-Length`、包管理器元数据或官方发布清单确认单文件大小。
4. 大小未知时先用 HEAD/元数据检查；服务端不提供长度时使用带 100 MB 上限的流式读取。
5. 超过上限立即中止并记录原因，不使用拆包方式规避限制。

开发依赖与角色资源分别处理：

- 开发依赖包括浏览器、编译器、runtime、测试工具和安装包。第一阶段复用已安装 Windows Chrome，不下载 Playwright Chromium/Chrome，也不安装 Unity Editor。
- 角色资源包括当前选择模型的 `.skel`、`.atlas`、atlas PNG 和按需生成的 sheet。资源按选择下载，每个网络对象仍受 100 MB 上限约束；永久模式允许多个已使用资源逐步累计。

所有获取日志至少保存来源、license 判定、声明大小或实收字节数、SHA-256 和结果。

当前 PLib 依赖预检结果：官方 PLib 4.25（U59），MIT License，`PLib.dll` 为 344,576 B。仓库保存 `PLIB-LICENSE.txt`。该依赖远低于 100 MB，Mod 通过现有 ONI/PLib 环境引用，不引入浏览器安装包。
...
```text
Documents/Klei/OxygenNotIncluded/mods/config/AmiyaDuplicantMod/
├─ config.json
├─ cache-index.json
├─ assets/
│  └─ <调用方提供的安全相对路径>/...
...
- `PrtsAssetClient` 使用 64 MiB 单文件上限，严格低于 100 MB 硬约束；同时检查响应 `Content-Length` 和流式实收字节数。
...
- 全量 Mod DLL 编译成功，`AmiyaDuplicantMod.dll` 当前为 241,664 B。
...
`AmiyaDuplicantOverlay.cs` 已接入计划：Begin 和 End 使用非循环轨道，主体按计划循环，Death 主体非循环。上述代码路径已经过源码检查；网页真实资源播放和游戏内动作同步仍需单独验证。
...
| ANI-002 | Move | 阿米娅基建 Move | 名称映射命中 `Move`；真实动画骨骼或 canvas checksum 前进 | 网页与纯逻辑均已验证 |
| ANI-003 | Work | 阿米娅基建 | `work` 输入命中 `Interact` | 名称映射已验证；真实资源待验证 |
| ANI-004 | Sleep/Sit | 阿米娅基建 | `sleep` 命中 `Sleep`；`sit` 命中 `Sit` | Sleep 网页已验证；Sit 纯逻辑已验证 |
...
游戏内显示名称为 `Arknights Operators（明日方舟干员）`。开发目录和 DLL 继续使用 `AmiyaDuplicantMod` 兼容标识。

在 Windows PowerShell 中通过 WSL 构建并安装：

```powershell
wsl.exe -- bash -lc "cd /mnt/c/Users/element/jingsai/arknights_oni_mod/arknights_oni_mod_work/AmiyaDuplicantMod && ./build.sh && ./install_local.sh"
```
