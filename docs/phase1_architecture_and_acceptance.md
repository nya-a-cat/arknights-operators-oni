...
`OperatorAnimationMapper.cs` 已实现动作分类、精确匹配、子串匹配、首动画降级、模型角色选择、手动动作优先级和 `OperatorAnimationPlan`。`OperatorAnimationMapperTests.cs` 的 40 项断言已在 2026-07-15 重新运行，输出为：

```text
OperatorAnimationMapperTests: 40 passed
...
通过 Steam 启动 ONI，进入“模组”并启用 `Arknights Operators（明日方舟干员）`，按游戏提示重启。模组管理页 Options 可设置资源策略和全局默认外观；进入存档后选中复制人并按 `Ctrl+F8` 可单独选择干员、皮肤和模型，`Ctrl+Shift+F8` 打开全局默认。更新源码后重复执行同一条命令即可覆盖本地安装文件。当前机器直接运行 `OxygenNotIncluded.exe` 会经过 Steam bootstrap 并产生一次错误的 Mod Safe Mode 判定，Steam 客户端启动路径已连续验证正常。
